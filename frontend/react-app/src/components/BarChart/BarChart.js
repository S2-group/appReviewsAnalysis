...
export default class BarChart extends React.Component {
  ...
  _defineAxis(props){
    ...
    this.x = d3.scaleBand().range([0, props.width]).round(0.1);
    this.y = d3.scaleLinear().range([props.height, 0]);

    this.x.domain(props.data.map(d => d.text));
    this.y.domain([0, (this.props.ymax) ? this.props.ymax : d3.max(props.data, d => d.value)]);
